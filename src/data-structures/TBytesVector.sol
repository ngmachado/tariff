...
library TBytesVector {
        ...
        returns (Vector memory vector)
    {
        ...
        vector.basePointer = AllocatorFactory.allocate(allocatorType, slot, initialCapacity * 32);
    ...
    function _setAt(Vector memory vector, uint256 index, bytes memory data) private {
        ...
        for (uint256 i = 0; i < (data.length + 31) / 32; i++) {
            ...
            AllocatorFactory.store(vector.allocator, bytes32(uint256(contentPointer) + i * 32), word);
        }
    }
    ...
    function _getAt(Vector memory vector, uint256 index) private view returns (bytes memory) {
        ...
        for (uint256 i = 0; i < (length + 31) / 32; i++) {
            uint256 word = AllocatorFactory.load(vector.allocator, bytes32(uint256(contentPointer) + i * 32));
            assembly {
    ...
    function _resize(Vector memory vector, uint256 newCapacity) private {
        bytes32 slot = keccak256(abi.encodePacked("TBytesVector_resize", vector.basePointer, _getNextNonce()));
        bytes32 newPointer = AllocatorFactory.allocate(vector.allocator, slot, newCapacity * 32);
    ...
    function _setAtPointer(
        ...
        bytes32 dataPointer = bytes32(uint256(basePointer) + index * 32 * ((data.length + 31) / 32 + 1));
        ...
        for (uint256 i = 0; i < (data.length + 31) / 32; i++) {
            ...
            AllocatorFactory.store(allocator, bytes32(uint256(contentPointer) + i * 32), word);
        }
    }

    function _getDataPointer(Vector memory vector, uint256 index) private pure returns (bytes32) {
        // Calculate slots needed for each element (1 for length + 1 for data = 2 slots)
        uint256 slotsPerElement = 2;
        return bytes32(uint256(vector.basePointer) + index * 32 * slotsPerElement);
    }
     ...
     */
    function readFrom(AllocatorFactory.AllocatorType allocatorType, bytes32 basePointer, uint256 index)
        internal
        view
